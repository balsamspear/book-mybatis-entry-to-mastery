import mybatis.entry.to.mastery.chapter07.mapper.RoleMapper;
import mybatis.entry.to.mastery.chapter07.mapper.UserMapper;
...
public class CacheTest extends BaseMapperTest {
    ...
    public void testL1Cache() {
        ...
        // 这里不使用二级缓存，所以把总开关关闭
        getSqlSession().getConfiguration().setCacheEnabled(false);
    ...
    public void testSelectNoCache() {
        ...
        try {
            ...
            UserMapper userMapper = sqlSession.getMapper(UserMapper.class);
            //调用 selectById 方法，查询 id = 1 的用户
            user1 = userMapper.selectByIdNoCache(1L);
            //对当前获取的对象重新赋值
            user1.setUserName("New Name");
            //再次查询获取 id 相同的用户
            SysUser user2 = userMapper.selectByIdNoCache(1L);
            //虽然我们没有更新数据库，但是这个用户名和我们 user1 重新赋值的名字相同了
            Assert.assertNotEquals("New Name", user2.getUserName());
            //不管如何，user2 和 user1 完全就是同一个实例
            Assert.assertNotEquals(user1, user2);
        } finally {
            //关闭当前的 sqlSession
            sqlSession.close();
        ...
        try {
            ...
            Assert.assertEquals(0, i);
            //获取 user3
            SysUser user3 = userMapper.selectByIdNoCache(1L);
            //这里的 user2 和 user3 是两个不同的实例
            Assert.assertNotEquals(user2, user3);
        } finally {
            //关闭 sqlSession
    ...
    public void testL2Cache() {
        ...
        try {
            ...
            //不管如何，role2 和 role1 完全就是同一个实例
            Assert.assertEquals(role1, role2);
        } finally {
            //关闭当前的 sqlSession
            sqlSession.close();
        }
        role1.setRoleName("asdfasdf");
        System.out.println("开启新的 sqlSession");
        //开始另一个新的 session
        sqlSession = getSqlSession();
        try {
    ...
    public void testDirtyData(){
        ...
        try {
            ...
            SysRole role = roleMapper.selectById(2L);
            //Assert.assertEquals("普通用户", user.getRole().getRoleName());
            Assert.assertNotEquals("普通用户", user.getRole().getRoleName());
